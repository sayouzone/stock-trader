...
def compute_all(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # 추가: 변동성 지표
    r["Volatility_21d"] = c.pct_change().rolling(21).std() * np.sqrt(252)
    r["Volatility_63d"] = c.pct_change().rolling(63).std() * np.sqrt(252)
